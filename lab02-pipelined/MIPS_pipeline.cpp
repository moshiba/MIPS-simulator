    ...
    while (1) {
        ...
        if(state.ID.nop == 0){
            ...
            if((opcode != 0x2) && (opcode != 0x3)){ //not a j-type
                newState.EX.Rs = bitset<5>((state.ID.Instr.to_ullong() >> 21) & 0x1F);
                newState.EX.Rt = bitset<5>((state.ID.Instr.to_ullong() >> 16) & 0x1F);
                newState.EX.Read_data1 = myRF.readRF(newState.EX.Rs);
                newState.EX.Read_data2 = myRF.readRF(newState.EX.Rt);
                newState.EX.Imm = bitset<16>((state.ID.Instr.to_ullong() & 0xFFFF));
                if(newState.EX.is_I_type == 1){
                    newState.EX.Wrt_reg_addr = newState.EX.Rt;
                    //Rt is the destination register for I-type
                }
                else{
                    newState.EX.Wrt_reg_addr = bitset<5>((state.ID.Instr.to_ullong() >> 11) & 0x1F);
